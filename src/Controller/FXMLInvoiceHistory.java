 ...
 * @author TechCare
 */
public class FXMLInvoiceHistory implements Initializable {
    ...
    private void init() {
        dao = new DataAccessObject();
        labelRESID.setText(FXMLHistory.RESID_history);
        query = "select * from Reservation where RESID = '"+ FXMLHistory.RESID_history +"'";
        Reservation res  = dao.getReservationDataByRESID(query);
        labelROOMID.setText(res.getROOMID());
        labelPassport.setText(res.getPASSPORT());
        
        query = "select * from Guest where passport = '"+ res.getPASSPORT() +"'";
        Guest guest = dao.getGuestDataByPASSPORT(query);
        ...
        Bill bill = dao.getBillByRESID(FXMLHistory.RESID_history);
        labelGetDateTime.setText(bill.getPaymentDate());
        labelResPrice.setText(String.valueOf(res.getResPrice()) + " $");
        RESprice = res.getResPrice(); // kieu int 
    }
    ...
    private void print(Node node) {
        ...
        if (job != null) {
            ...
            if (printed) {
                // End the printer job
                job.endJob();
                FXMLHistory.stageInvoice.close();
            } else {
                checkInvoice = false;
            }
        } else {
